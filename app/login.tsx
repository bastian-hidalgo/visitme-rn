...
export default function LoginScreen() {
  ...
  const handleGoogleStatusChange = useCallback(
    (
      status: GoogleLoginButtonStatus,
      details?: { errorMessage?: string | null },
    ) => {
      if (status === "loading") {
        setIsGoogleLoading(true);
        setErrorMessage(null);
        addActionBreadcrumb("Google login started");
        return;
      }
      setIsGoogleLoading(false);
      if (status === "error") {
        const errorMsg = details?.errorMessage ?? "Error con Google.";
        recordError(new Error(errorMsg), "LoginScreen.googleLogin");
        setErrorMessage(errorMsg);
      }
      if (status === "success") {
        addBreadcrumb("Google login successful", "auth");
      }
    },
    [],
  );
  ...
  const handleAppleStatusChange = useCallback(
    (
      status: AppleLoginButtonStatus,
      details?: AppleLoginStatusChangeDetails,
    ) => {
      if (status === "loading") {
        setIsAppleLoading(true);
        setErrorMessage(null);
        addActionBreadcrumb("Apple login started");
        return;
      }
      setIsAppleLoading(false);
      if (status === "error") {
        const errorMsg = details?.errorMessage ?? "Error con Apple.";
        recordError(new Error(errorMsg), "LoginScreen.appleLogin");
        setErrorMessage(errorMsg);
      }
      if (status === "success") {
        addBreadcrumb("Apple login successful", "auth");
      }
    },
    [],
  );
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.mainContent}>
          ...
          <View style={styles.welcomeContainer}>
            ...
            >
              <ThemedText style={styles.welcomeSubtitle}>
                Reserva espacios comunes, gestiona invitaciones seguras, recibe
                encomiendas y mantente conectado con tu comunidad.
              </ThemedText>
            </MotiView>
          </View>
          ...
          >
            <Pressable style={styles.ctaButton} onPress={openLoginOptions}>
